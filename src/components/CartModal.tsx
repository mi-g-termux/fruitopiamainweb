...
const getAutomaticPaymentCredentialError = (settings: any, method: string): string => {
  const requiredFields: Record<string, Array<[string, string]>> = {
    bKashAuto: [['bKashAppKey', 'App Key'], ['bKashAppSecret', 'App Secret'], ['bKashUsername', 'Username'], ['bKashPassword', 'Password']],
    NagadAuto: [['nagadMerchantId', 'Merchant ID'], ['nagadMerchantPrivateKey', 'Merchant Private Key']],
    PayPal: [['paypalClientId', 'Client ID'], ['paypalClientSecret', 'Client Secret']],
    Stripe: [['stripePublicKey', 'Publishable Key'], ['stripeSecretKey', 'Secret Key']],
    SSLCommerz: [['sslCommerzStoreId', 'Store ID'], ['sslCommerzStorePassword', 'Store Password']],
    Razorpay: [['razorpayKeyId', 'Key ID'], ['razorpayKeySecret', 'Key Secret']],
    Paytm: [['paytmMerchantId', 'Merchant ID'], ['paytmMerchantKey', 'Merchant Key']],
    UPI: [['upiId', 'UPI ID / VPA']],
    JazzCash: [['jazzCashMerchantId', 'Merchant ID'], ['jazzCashPassword', 'Password'], ['jazzCashIntegritySalt', 'Integrity Salt']],
    Easypaisa: [['easypaisaStoreId', 'Store ID'], ['easypaisaHashKey', 'Hash Key']],
    PayFast: [['payFastMerchantId', 'Merchant ID'], ['payFastMerchantKey', 'Merchant Key']],
  };
  ...
  const handleCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (AUTOMATIC_PAYMENT_METHODS.includes(paymentMethod)) {
          ...
          try {
            const res = await fetch('/api/nagad/create-payment', {
              ...
              body: JSON.stringify({
                ...
                merchantPrivateKey: paymentSettings.nagadMerchantPrivateKey,
                privateKey: paymentSettings.nagadMerchantPrivateKey,
                publicKey: paymentSettings.nagadPublicKey,
        ...
        if (paymentMethod === 'Paytm' && paymentSettings.paytmMerchantId && paymentSettings.paytmMerchantKey) {
          try {
            const orderId = `QF-${Date.now()}`;
            const res = await fetch('/api/paytm/initiate', {
              ...
              body: JSON.stringify({
                amount: grandTotal.toFixed(2),
                orderId,
                merchantId: paymentSettings.paytmMerchantId,
                merchantKey: paymentSettings.paytmMerchantKey,
        ...
        if (paymentMethod === 'JazzCash' && paymentSettings.jazzCashMerchantId) {
          try {
            const orderId = `QF-${Date.now()}`;
            const res = await fetch('/api/jazzcash/initiate', {
              ...
              body: JSON.stringify({
                amount: grandTotal.toFixed(2),
                orderId,
                merchantId: paymentSettings.jazzCashMerchantId,
                password: paymentSettings.jazzCashPassword,
                integritySalt: paymentSettings.jazzCashIntegritySalt,
        ...
        if (paymentMethod === 'Easypaisa' && paymentSettings.easypaisaStoreId) {
          try {
            const orderId = `QF-${Date.now()}`;
            const res = await fetch('/api/easypaisa/initiate', {
              ...
              body: JSON.stringify({
                amount: grandTotal.toFixed(2),
                orderId,
                storeId: paymentSettings.easypaisaStoreId,
                hashKey: paymentSettings.easypaisaHashKey,
        ...
        if (paymentMethod === 'PayFast' && paymentSettings.payFastMerchantId && paymentSettings.payFastMerchantKey) {
          try {
            const orderId = `QF-${Date.now()}`;
            const res = await fetch('/api/payfast/initiate', {
              ...
              body: JSON.stringify({
                ...
                orderId,
                productName: siteSettings?.storeName || 'Order',
                merchantId: paymentSettings.payFastMerchantId,
                merchantKey: paymentSettings.payFastMerchantKey,
                passphrase: paymentSettings.payFastPassphrase,
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {([
                    ...
                    { id: 'Paytm',       fallbackLabel: 'Paytm',            icon: <CreditCard className="w-4 h-4" />,  enabled: paymentSettings.paytmEnabled === true,      displayName: (paymentSettings as any).paytmDisplayName,     logoUrl: (paymentSettings as any).paytmLogoImageUrl },
                    { id: 'UPI',         fallbackLabel: 'UPI / QR',         icon: <CreditCard className="w-4 h-4" />,  enabled: paymentSettings.upiManualEnabled === true,  displayName: (paymentSettings as any).upiDisplayName,       logoUrl: (paymentSettings as any).upiLogoImageUrl },
                    { id: 'JazzCash',    fallbackLabel: 'JazzCash',         icon: <CreditCard className="w-4 h-4" />,  enabled: paymentSettings.jazzCashEnabled === true,   displayName: (paymentSettings as any).jazzCashDisplayName,  logoUrl: (paymentSettings as any).jazzCashLogoImageUrl },
                    { id: 'Easypaisa',   fallbackLabel: 'Easypaisa',        icon: <CreditCard className="w-4 h-4" />,  enabled: paymentSettings.easypaisaEnabled === true,  displayName: (paymentSettings as any).easypaisaDisplayName, logoUrl: (paymentSettings as any).easypaisaLogoImageUrl },
                    { id: 'PayFast',     fallbackLabel: 'PayFast',          icon: <CreditCard className="w-4 h-4" />,  enabled: paymentSettings.payFastEnabled === true,    displayName: (paymentSettings as any).payFastDisplayName,   logoUrl: (paymentSettings as any).payFastLogoImageUrl },
